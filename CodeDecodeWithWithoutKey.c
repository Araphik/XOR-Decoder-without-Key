...
#define ALPHABET_RU "АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдежзийклмнопрстуфхцчшщъыьэюяЁё"
...
void insertionSort(int arr1[], int arr2[], int n) 
{
	...
	for (i = 1; i < n; i++) 
	{
		...
		// относительно текущей позиции j
...
void Coding()
{
	Symbol Sym;
	File F;
	F.Start = fopen("Start.txt", "rb");;//исходный текст
	...
	int buf1 = ftell(F.Start);//ищем длину исходного текста
	fseek(F.Start, 0, SEEK_SET);

	fseek(F.Key, 0, SEEK_END);
	int buf2 = ftell(F.Key);//ищем длину ключа
	...
	while (ftell(F.Start) != buf1)
	{
		...
		Sym.File ^= Sym.Key;//посимвольное шифрование
...
void Decoding_Key()
{
	...
	int buf1 = ftell(F.Coded);//ищем длину исходного текста
	...
	while (ftell(F.Coded) != buf1)
	{
		...
		Sym.File ^= Sym.Key;//посимвольное дешифрование
...
{
	long long freq[256] = { 0 }; // массив для хранения частот символов
	float coincidence = 0.0;//совпадение
...
int LenKey()
{
	...
	fseek(F.Coded, 0, SEEK_END);
	int buf1 = ftell(F.Coded);//длина зашифрованного текста
	fseek(F.Coded, 0, SEEK_SET);

	float* coincidence_index_avg;//динамеческий массив для индексов совпадения
	coincidence_index_avg = (float*)malloc(MAX_SIZE * sizeof(float));
	memset(coincidence_index_avg, 0, MAX_SIZE);

	for (int key_len = 1; key_len <= MAX_SIZE; key_len++)//перебор длины ключа
	{
		int count = 0;//для подсчета количества блоков
		coincidence_index_avg[key_len - 1] = 0.0;//перебор различного сдвига
		for (int i = 0; i < key_len; i++)
		{
			unsigned char block[BLOCK_SIZE] = { 0 };//массив для хранения блока
			int block_count = 0;//счетчик блоков
		...
		coincidence_index_avg[key_len - 1] /= count;//среднее количество совпадений символов
...
void FindKey(int Length_Key)
{
	...
	{
		unsigned char* block;//для анализирования количества символов
		...
		for (int j = i; j < buf1; j += Length_Key)
		{
			...
			block[index] ^= ' ';//xor c самым популярным символом
			if (block[index] == 0)
				block[index] = '0';
			index++;
		}
		...
		rewind(F.Coded);
		block[index] = '\0';
		unsigned char most_common = 0;//самый популярный символ зашифрованного текста
		int most_count = 0;//его индекс
		int j = 0;
...
void Decoding_NoKey()
{
	Symbol Sym;
	File F;
	F.Coded = fopen("Coded.txt", "rb");//Открываем закодированное сообщение

	Sym.File = '\0', Sym.Key = '\0';

	fseek(F.Coded, 0, SEEK_END);
	int buf1 = ftell(F.Coded);//узнаем длину закодированного сообщения
	fseek(F.Coded, 0, SEEK_SET);

	int Length_Key = 0;
	Length_Key = LenKey();//длина ключа
	printf("Длина ключа = %d\n", Length_Key);

	FindKey(Length_Key);//находим ключ
...
void Menu()
{
	...
	while (1)
	{
		...
		printf("------------------------------------\n");
		printf("|                                  |\n");
		printf("|         Что вы выберете?         |\n");
		printf("|                                  |\n");
		printf("|  1) Шифрование                   |\n");
		printf("|                                  |\n");
		printf("|  2) Дешифрование с ключом        |\n");
		printf("|                                  |\n");
		printf("|  3) Дешифрование без ключа       |\n");
		printf("|                                  |\n");
		printf("|  4) Выход                        |\n");
		printf("|                                  |\n");
		printf("------------------------------------\n");
